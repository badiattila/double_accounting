...
class AIAssistTests(TestCase):
	def setUp(self):
		...
		self.acc_supplies = Account.objects.create(code="5000", name="Office Supplies", type=AccountType.EXPENSE, normal_debit=True)
		self.acc_travel = Account.objects.create(code="5200", name="Travel", type=AccountType.EXPENSE, normal_debit=True)
		self.acc_misc = Account.objects.create(code="5999", name="Misc", type=AccountType.EXPENSE, normal_debit=True)
	...
	def test_fallback_returns_default_when_no_model(self):
		...
		if MODEL_PATH.exists():
			MODEL_PATH.unlink()
		code, conf = predict_account_code(payee="AMAZON EU", narrative="cables", amount=Decimal("12.99"))
	...
	def test_training_creates_model_file_and_predicts(self):
		...
		qs = EntryLine.objects.filter(account__code__in=["5000","5200"]) # only the debit lines interest
		...
		code, conf = cat.predict(payee="AMAZON", narrative="notebook bulk", amount=25.00)
	...
	def test_predict_service_with_model(self):
		...
		train_from_ledger(qs)
		code, conf = predict_account_code(payee="paper shop", narrative="office pads", amount=12.0)

...
class AccountingModelTests(TestCase):
    def setUp(self):
        self.cash = Account.objects.create(
            code="1000", name="Cash", type=AccountType.ASSET, normal_debit=True
        )
        self.rev = Account.objects.create(
            code="4000", name="Revenue", type=AccountType.INCOME, normal_debit=False
        )
    ...
    def test_transaction_needs_two_lines(self):
        ...
        EntryLine.objects.create(
            transaction=tx, account=self.cash, debit=Decimal("10.00")
        )
        with self.assertRaises(ValidationError):
            tx.full_clean()
    ...
    def test_transaction_balanced_validation(self):
        ...
        EntryLine.objects.create(
            transaction=tx, account=self.rev, credit=Decimal("9.99")
        )
        with self.assertRaises(ValidationError):
            tx.full_clean()
    ...
    def test_base_amount_sign_logic(self):
        tx = Transaction.objects.create(journal=self.journal, tx_date=timezone.now())
        debit_line = EntryLine.objects.create(
            transaction=tx, account=self.cash, debit=Decimal("20.00")
        )
        credit_line = EntryLine.objects.create(
            transaction=tx, account=self.rev, credit=Decimal("20.00")
        )
